...
def crawler(url):
    ...
    for article in soup.find_all(class_="r-ent"):
        ...
        link =  "https://www.ptt.cc" + article.find(class_="title").a['href']

        results.append({"title":title, "author":author, "limk":link})
    
url = 'https://www.ptt.cc/bbs/Baseball/index3.html'

while True:
    crawler(url)

    soup = BeautifulSoup(requests.get(url).text, "html.parser")
    res = soup.find_all(class_="btn wide")
    if len(res) < 4:
        break
    next_url = "https://www.ptt.cc" + res[1]['href']
    url=next_url
